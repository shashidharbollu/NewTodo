...
module.exports.createHabit = async function(req, res){
    ...
    try{
        ...
        if(habit){
            ...
            req.flash('success' , 'hurray! new Todo is now created');
            return res.redirect('/');
        }
...
module.exports.toogleStatus = async function(req, res){
    ...
    try{
        ...
        if(!habit){
            console.log("oops! theres is no such entry");
            return res.redirect('/');
        }else{
            ...
            dates.find((item, index) =>{
                if(item.date == date){
                    if(item.complete === 'yes'){
                        ...
                        item.complete = 'none';
                        message = 'Changed from Not Done to Not Marked'
...
module.exports.removeHabit = async (req, res) => {
    try {
        ...
        // Check if the request is coming from a browser environment
        if (typeof window !== 'undefined' && window.confirm) {
            // Use a confirm dialog to ask the user for confirmation
            ...
            if (!confirmation) {
                ...
                req.flash('info', 'Todo deletion cancelled.');
                return res.redirect('/');
            }
        }

        // If the user confirms or if running on the server side, proceed with the deletion
